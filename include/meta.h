...
#define field_parent_ptr(Container, field_name, pointer_to_field)                                                      \
    ((Container *)(((uintptr_t)pointer_to_field) - offsetof(Container, field_name)))

// Takes whatever `value` is and returns it as a string literal
#define comptime_stringify(value) #value

// Returns the inner `value` of the given macro as a string. Can be used to convert things like numbers
// or `__LINE__` to string form.
