...
static void spi_transact(void) {
    // Trigger start of user-defined transaction and wait until it's done.
    GPSPI2.cmd.usr = 1;
...
void spi_controller_init(
    ...
    if (!io_is_peripheral(ec, sclk_pin)
     && GPIO.func_out_sel_cfg[sclk_pin].out_sel != SPI2_CK_PAD_OUT_IDX) {
        logkf(LOG_ERROR, "SCLK pin (%{d}) already in use (%{d}, %{d})", sclk_pin, ec->cause, ec->location);
        return;
    } 
    else 
    if (!io_is_peripheral(ec, mosi_pin)
     && GPIO.func_out_sel_cfg[mosi_pin].out_sel != SPI2_D_PAD_OUT_IDX) {
        logkf(LOG_ERROR, "MOSI pin (%{d}) already in use (%{d}, %{d})", sclk_pin, ec->cause, ec->location);
        return;
    }
    else
    if (!io_is_peripheral(ec, miso_pin)) {
        logkf(LOG_ERROR, "MISO pin (%{d}) already in use (%{d}, %{d})", sclk_pin, ec->cause, ec->location);
        return;
    }
    else 
    if (!io_is_peripheral(ec, ss_pin)
     && GPIO.func_out_sel_cfg[ss_pin].out_sel != SPI2_CS_PAD_OUT_IDX) {
        logkf(LOG_ERROR, "SS pin (%{d}) already in use (%{d}, %{d})", sclk_pin, ec->cause, ec->location);
        return;
    }

    // Clock configuration and reset
    clkconfig_spi2(bitrate, true, true);


    irq_ch_set_isr(ETS_SPI2_INTR_SOURCE, (isr_t)spi_isr);
    irq_ch_enable(ETS_SPI2_INTR_SOURCE);
    // GPSPI2.dma_int_ena.trans_done = true;

    spi_config_apply();

    // Set pins to GPIO function and enable input for MISO
    IO_MUX.gpio[sclk_pin] = (io_mux_gpio_reg_t){
        .mcu_sel = 1,
    };
    IO_MUX.gpio[miso_pin] = (io_mux_gpio_reg_t){.mcu_sel = 1, .fun_ie = true};
    IO_MUX.gpio[mosi_pin] = (io_mux_gpio_reg_t){.mcu_sel = 1};
    IO_MUX.gpio[ss_pin]   = (io_mux_gpio_reg_t){.mcu_sel = 1};
    ...
    GPIO.func_out_sel_cfg[sclk_pin] = (gpio_func_out_sel_cfg_reg_t){
        ...
        .oen_sel     = false,
        .out_inv_sel = false,
        .out_sel     = SPI2_CK_PAD_OUT_IDX,
    };
    GPIO.func_out_sel_cfg[mosi_pin] = (gpio_func_out_sel_cfg_reg_t){
        .oen_inv_sel = false,
        .oen_sel     = false,
        .out_inv_sel = false,
        .out_sel     = SPI2_D_PAD_OUT_IDX,
    };
    GPIO.func_out_sel_cfg[ss_pin] = (gpio_func_out_sel_cfg_reg_t){
        .oen_inv_sel = false,
        .oen_sel     = false,
        .out_inv_sel = false,
        .out_sel     = SPI2_CS_PAD_OUT_IDX,
    };

    GPIO.func_in_sel_cfg[SPI2_Q_PAD_IN_IDX] = (gpio_func_in_sel_cfg_reg_t){
        .in_sel     = miso_pin,
        .in_inv_sel = false,
        .sig_in_sel = true,
    };
...
static void spi_controller_transfer_generic(badge_err_t *ec, int spi_num, void *buf, size_t len) {
    ...
    size_t const num_words = data_buf_len / sizeof(GPSPI2.data_buf[0]);
